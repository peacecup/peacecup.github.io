$.extend({
  ...
  toc: function (toc) {
    ...
    toc.find('a[href="#' + titles[0].id + '"]').parent().addClass(
      'active');
    return {
      fixed: function (top) {
      ...
      actived: function (top) {
        titles.each(function () {
          ...
          if (top > $(this).offset().top - headerH) {
            ...
            var active = toc.find('a[href="#' + $(this).attr('id') +
              '"]').parents('.post-toc-item');
            active.addClass('active');
          }
        });
        if (top < titles.eq(0).offset().top) {
          toc.find('li.active').removeClass('active');
          var active = toc.find('a[href="#' + titles.eq(0).attr('id') +
            '"]').parent();
          active.addClass('active');
        }
      },
  ...
  reward: function () {
    ...
    if (rewardToggle.length != 0) {
      rewardToggle.change(function () {
        rewardCode.attr('src', this.checked ? this.dataset.alipay :
          this.dataset.wechat);
      });
    }
  },
