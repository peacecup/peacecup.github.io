(function (w, d) {
  var body = d.body,
    ...
    scrollId,
    stopScroll = w.cancelAnimationFrame,
    scrollSpeed = 200 / (1000 / 60),
    forEach = Array.prototype.forEach,
    even = ('ontouchstart' in w &&
      /Mobile|Android|iOS|iPhone|iPad|iPod|Windows Phone|KFAPWI/i.test(
        navigator.userAgent)) ? 'touchstart' : 'click',
    isWX = /micromessenger/i.test(navigator.userAgent),
    isPhone = /(iPhone|iPod|Android|ios)/i.test(navigator.userAgent),
    noop = function () {},
    offset = function (el) {
      var x = el.offsetLeft,
        y = el.offsetTop;
    ...
    docEl = d.documentElement || body;
  var Blog = {
    goTop: function (iTarget) {
      clearInterval(docEl.timer);
      docEl.timer = setInterval(function () {
        var iStatus = docEl.scrollTop;
        if (iStatus > iTarget) {
          var iSpeed = Math.floor((iTarget - iStatus) * 0.15);
        } else {
          var iSpeed = Math.ceil((iTarget - iStatus) * 0.15);
        }
        if (Math.abs(iTarget - iStatus) < 0.5) {
          clearInterval(docEl.timer);
        } else {
          docEl.scrollTop = iStatus + iSpeed;
        }
      }, 13);
    },
    ...
    toc: (function () {
      ...
      var bannerH = $('.post-header').clientHeight,
        ...
        titles = $('#post-content').querySelectorAll(
          'h1, h2, h3, h4, h5, h6')
      toc.querySelector('a[href="#' + titles[0].id + '"]').parentNode.classList
        .add('active');
      return {
        fixed: function (top) {
          top >= bannerH - headerH ? toc.classList.add('fixed') : toc
            .classList.remove('fixed');
        },
        actived: function (top) {
          for (i = 0, len = titles.length; i < len; i++) {
            var plug = parseInt(getStyle(titles[i], 'marginBottom'));
    ...
    modal: function (target) {
      ...
      this.toggle = function () {
        return _this.$modal.classList.contains('in') ? _this.hide() :
          _this.show();
      }
    ...
    reward: function () {
      ...
      if ($rewardToggle) {
        $rewardToggle.addEventListener('change', function () {
          $rewardCode.src = this.checked ? this.dataset.alipay : this
            .dataset.wechat
        })
      }
    },
    waterfall: function () {
      ...
      forEach.call($$('.waterfall'), function (el) {
        ...
        forEach.call(childs, function (item) {
          ...
          item.style.cssText = 'top:' + columns[i] + 'px;left:' +
            (i > 0 ? '50%' : 0);
    ...
    tocScroll: function (target) {
      tagName = target.tagName.toLowerCase(),
        titles = $('#post-content').querySelectorAll(
          'h1, h2, h3, h4, h5, h6');
      if (tagName === 'a') {
        var id = target.href.replace(/http.+\#/, '');
        for (var i = 0; i < titles.length; i++) {
          if (titles[i].id === id) {
            var top = offset(titles[i]).y;
            if (i === titles.length - 1) {
              top = docEl.scrollHeight - docEl.clientHeight + 10;
            }
            Blog.goTop(top - 10);
          }
        }
  ...
  toc && toc.addEventListener(even, function (e) {
    e.preventDefault();
    Blog.tocScroll(e.target);
  }, false);
