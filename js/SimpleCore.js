...
var SimpleCore = {
  ...
  init: function (params) {
    ...
    $(document).on('click', '.btn-weixin-mp', function (e) {
      ...
      if (SimpleCore.wxMpImg != '') {
        SimpleCore.alert('更多精彩扫扫这里',
          '<img style="width:160px;background:#fff;" src="' +
          SimpleCore.wxMpImg + '">');
      } else {
        SimpleCore.alert('未开通自定义功能',
          '<h4 style="text-align: center;margin: 0">联系博主试试看 ：）</h4>');
      }
    });
    $(document).on('click', '.btn-thumbs-up', function (e) {
      e.preventDefault();
      if (SimpleCore.donateImg != '') {
        SimpleCore.alert('扫码请我喝可乐',
          '<img style="width:160px;background:#fff;" src="' +
          SimpleCore.donateImg + '">');
      } else {
        SimpleCore.alert('未开通自定义功能',
          '<h4 style="text-align: center;margin: 0">联系博主试试看 ：）</h4>');
      }
  ...
  scrollCallback: function () {
    ...
    if ($('body').hasClass('single')) {
      SimpleCore.headerShow = (top < 100 || (SimpleCore.prevTop - top) > 0) ?
        true : false;
  ...
  alert: function (title, msg) {
    ...
    var html = '<div id="' + id + '" class="notice-item">' +
      ...
      '<p><h3 style="text-align: center;margin:0 0 10px 0">' + title +
      '</h3>' + msg + '</p></div>';
    var notice = $('#notice');
    if (notice.length == 0) {
      $('<div id="notice"></div>').appendTo($('body'));
    }
    $(html).appendTo($('#notice')).on('click', '.notice-item-close',
      function () {
        $(this).parent().remove();
        return false;
      });
